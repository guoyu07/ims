...
<script type="text/javascript">
	...
	var teacher = '${ teacher }';
	var status = '${ status }';
	var transStartDate = '${ transStartDate }'; // 
	var transEndDate = '${ transEndDate }'; // 
	var checkStartDate = '${ checkStartDate }'; // 
	var checkEndDate = '${ checkEndDate }'; // 
	var month = '${ month }';
	var targetPageNum = '${ page }';
	
	var urlPre = 'checkRecordList?';
	function goSearch(url){
		var url = urlPre+'&page='+targetPageNum + $('form').serialize();
		location.href = url;
	}
</script>
</head>
...
<c:set var="preUrl" value="checkRecordList
	...
	<div class="static2_base">
		...
		<ul class="searchSub">	
			...
			<li><button class="btn btn-primary btn-xs" onclick="goSearch('checkRecordSearch?pageNum=0')" >搜索</button></li>
			<a href="#" class="btn btn-success btn-xs ml10" id="generate-sample-btn">生成抽检</a></li>
		</ul>
		</form>
		<table class="table table-hover table-bordered" id="memberListTab">
			...
			<tbody>
				...
				<tr><td><a href="<%=basePath %>piccheck/tranOpsDetailViewSearch?grandParentId=${ record.id }">${ func:formatDate(record.createTime) }</a></td>
					...
					<td><a class="btn btn-primary btn-xs"  href="<%=basePath %>piccheck/checkDetailRecordExport2Excel?parentId=${ record.id }">导出抽检结果</a></td>
				</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		success: function() {
			seajs.use(['lib/jquery', 'module/SampleCheck'], function($, sampleCheck) {
				...
				$('#search').on('click', function(e) {
					...
					location.href = location.origin + location.pathname + '?' + $('#search-form').serialize();
				});
				// 绑定生成抽检
				$("#generate-sample-btn").on('click', function(e) {
					var el = $(this);
					e.preventDefault();
					sampleCheck(window.basePath + 'piccheck/generateCheckRecord', {
						source: el.attr('id')
					});
				});
			});
