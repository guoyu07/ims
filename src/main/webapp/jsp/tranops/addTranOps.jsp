		...
		<div class="header">
			...
			<div class="photo-footor">
				...
				<select name="realType" class="question-type"><option value="">题型</option>
					...
					<c:forEach var="enumSubject" items="${ enumSubjects }">
					<option value="${ enumSubject.id }" <c:if test="${ enumSubject.id == tranOps.realType }">selected = "selected"</c:if>>${ enumSubject.chineseName }</option>
					</c:forEach>
				</select>
				<select name="realLearnPhase" class="grade"><option value="">学段</option>
					<c:set var="enumSubjects" value="<%= EnumGradeType.values() %>"/>
					<c:forEach var="enumSubject" items="${ enumSubjects }">
					<option value="${ enumSubject.id }" <c:if test="${ enumSubject.id == tranOps.realLearnPhase }">selected = "selected"</c:if>>${ enumSubject.chineseName }</option>
					</c:forEach>
				</select>
			</div>
		</div>
		...
		success: function() {
			seajs.use(['lib/jquery', 'page/edit'], function($, edit) {
				edit.init('preview-area', {
					editArea: $('#edit-area'),
					submitUrl: window.basePath + 'tranops/addTranOps',
