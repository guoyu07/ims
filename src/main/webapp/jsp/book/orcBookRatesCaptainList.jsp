...
<body>
<c:set var="preUrl" value="orcBookRatesCaptainList
							?isbn=${ isbn }
							&operator=${ operator }
							&status=${ status }
							&name=${ name }
			...
			<tbody>
				<c:forEach var="orcbooksInfo" items="${ orcBookRatesInfoList }">
					<tr>
						...
						<td>${ orcbooksInfo.books.grade }</td>
						<td>${ orcbooksInfo.books.subject }</td>
						<td>${ orcbooksInfo.books.isbn }</td>
						<td>${ orcbooksInfo.books.publishingHouse }</td>
						<td>${ orcbooksInfo.orcBooks.operatorName }</td>
						<td>${ orcbooksInfo.tikuTeam.name }</td>
						<td>${ func:formatDate(orcbooksInfo.orcBooks.operatorStartime) }</td>
						<td>${ func:formatDate(orcbooksInfo.orcBooks.operatorEndtime) }</td>
						<td>${ orcbooksInfo.orcRate }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
