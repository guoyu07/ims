...
<body>
<c:set var="preUrl" value="recognitionList
							?fileId=${ fileId }
							&startTime=${ startTime }
							&endTime=${ endTime }&" />
	<div class="crumb">当前位置：识别列表</div>
	<div class="static2_base">
		<form>
			<ul class="searchSub">
				<li>图片ID<input type="text" value="${ fileId }" name="fileId" /></li>
				<li>识别日期<input class="form-control form-date" type="text" value="${ startTime }" name="startTime" data-date-format="yyyy-mm-dd 00:00:00">~<input class="form-control form-date" type="text" value="${ endTime }" name="endTime" data-date-format="yyyy-mm-dd 23:59:59"/></li>
				<li>
					<button class="btn btn-primary btn-xs">查询</button>
				</li>
				<li>
					<a class="btn btn-primary btn-xs" href="<%=basePath%>recognition/startRecognitionPicture">开始识别</a>
				</li>
			</ul>
		</form>
			...
			<tbody>
				<c:forEach var="question" items="${ recognitionList }">
					<tr>
						...
						<td><img src="<%=basePath %>${ question.recognitionPicture.filePath }" width="100" height="100"/></td>
