...
function getImg() {
	var callback = function(data) {
		var editorZone, imgHolder;
		editorZone = [EditWindow_body, EditWindow_answer, EditWindow_analysis][Part];
		imgHolder = $(editorZone).find('img').filter('[src$="img.png"]');
		$(ImgInput).hide();
		$(BodyMask).hide();
		$('#ImgPathLoc').val('');
		$('#ImgPathNet').val('');
		if(data.status === 0) {
			imgHolder.addClass('inmg').attr('src', data.result.imageUrl);
		} else {
			imgHolder.remove();
			alert(data.msg);
		}
    ...
    if(isImgLocal) {
    	MX.load({
    		...
    		version: '150528',
    		success: function() {
				seajs.use(['util/uploadFile'], function(uploader) {
					uploader($('#SelectImg')[0].files[0], {
						end: function(results) {
							var data, editorZone, imgHolder;
							data = results[0];
							callback(data);
						},
						error: function() {
...
if(document.getElementById("noinput_answer")) {
...
document.getElementById("Sub").onclick = function() {
    ...
    var result = {
    	...
    	realType: $('select[name=realType]').val(),
    	realLearnPhase: $('select[name=realLearnPhase]').val(),
    ...
    // 输入判定
  	...
  	if(result.subject.length === 0) {
  		alert("请选择学科后再提交");
  		return;
  	} else if(result.realType.length === 0) {
  		alert("请选择题型后再提交");
  		return;
  	} else if(result.realLearnPhase.length === 0) {
  		alert("请选择年级后再提交");
  		return;
  	}
	...
	doc.getElementById("realType_input").setAttribute("value",result.realType);
	doc.getElementById("realLearnPhase_input").setAttribute("value",result.realLearnPhase);
...
if(document.getElementById("pic-rec-other-sub"))
	document.getElementById("pic-rec-other-sub").onclick = function() {
	var doc = document;
	var xhr = new XMLHttpRequest();
	var pictureId = doc.getElementById("pic_id").innerText;
    xhr.onreadystatechange = function() {
	...
	doc.getElementById("pictureId_input").setAttribute("value",result.pictureId);
	doc.getElementById("pictureUrl_input").setAttribute("value",result.pictureUrl);
	doc.getElementById("target_input").setAttribute("value",result.target);
