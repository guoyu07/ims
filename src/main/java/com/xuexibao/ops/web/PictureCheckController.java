...
import com.xuexibao.ops.service.PictureCheckService;
import com.xuexibao.ops.util.DateUtils;


@Controller
@RequestMapping(value = "/piccheck")
public class PictureCheckController extends AbstractController {
	
	@Autowired
	protected PictureCheckService pictureCheckService;
	...
	public @ResponseBody ResponseResult generateCheckRecord(HttpServletRequest request, ModelMap model, String startTime, String endTime, Integer n, String teamIds) {
		try {
			...
			pictureCheckService.generateCheckRecord(startDate, endDate, n, teamIds, operator);
	    	return successJson();
		} catch (Exception e) {
			e.printStackTrace();
